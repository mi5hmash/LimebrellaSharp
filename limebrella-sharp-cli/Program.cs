...
async Task UnpackAll()
{
    ...
    await core.UnpackFilesAsync(inputRootPath, steamId, cts);
    cts.Dispose();
}
...
async Task PackAll()
{
    ...
    var inputRootPath = GetValidatedInputRootPath();
    await core.PackFilesAsync(inputRootPath, steamId, cts);
    cts.Dispose();
}
...
async Task ResignAll()
{
    ...
    if (string.IsNullOrEmpty(steamIdOutput))
        throw new ArgumentException("Output Steam ID is missing.");
    var inputRootPath = GetValidatedInputRootPath();
    await core.ResignFilesAsync(inputRootPath, steamIdInput, steamIdOutput, cts);
    cts.Dispose();
}
