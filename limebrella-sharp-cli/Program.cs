...
// Initialize APP_INFO
var appInfo = new MyAppInfo("limebrella_sharp_cli");
...
static void PrintHelp()
{
    ...
    var helpMessage = $"""
                       ...
                       Options:
                         ...
                         -sI <old_id>   Original Steam ID (used in resign mode)
                         -sO <new_id>   New Steam ID (used in resign mode)
                         -v             Verbose output
                         -h             Show this help message

                       Examples:
                         Unpack: {exeName} -m u -p "{inputPath}" -s 76561197960265729
                         Pack:   {exeName} -m p -p "{inputPath}" -s 76561197960265729
                         Resign: {exeName} -m r -p "{inputPath}" -sI 76561197960265729 -sO 76561197960265730
                       """;
...
void UnpackAll()
{
    ...
    core.UnpackFiles(inputRootPath, steamId, cts);
    cts.Dispose();
}
...
{
    var cts = new CancellationTokenSource();
    arguments.TryGetValue("-u", out var steamId);
    if (string.IsNullOrEmpty(steamId))
        throw new ArgumentException("Output Steam ID is missing.");
    var inputRootPath = GetValidatedInputRootPath();
    core.PackFiles(inputRootPath, steamId, cts);
    cts.Dispose();
}
...
void ResignAll()
{
    ...
    if (string.IsNullOrEmpty(steamIdOutput))
        throw new ArgumentException("Output Steam ID is missing.");
    var inputRootPath = GetValidatedInputRootPath();
    core.ResignFiles(inputRootPath, steamIdInput, steamIdOutput, cts);
    cts.Dispose();
}
