...
public static class LimeDeencryptor
{
    ...
    /// <param name="aesEncryptionPlatform">Specifies whether to use hardware-accelerated or software-based AES decryption.</param>
    private static void DeencryptData(Span<byte> inputData, Span<byte> encryptionKey, AesEncryptionPlatform aesEncryptionPlatform)
    {
        switch (aesEncryptionPlatform)
        {
    ...
    /// <param name="aesEncryptionPlatform">Specifies whether to use hardware-accelerated or software-based AES deencryption.</param>
    /// <returns>A task that represents the asynchronous deencryption operation.</returns>
    private static async Task DeencryptDataAsync(Memory<byte> inputData, Memory<byte> encryptionKey, AesEncryptionPlatform aesEncryptionPlatform)
    {
        switch (aesEncryptionPlatform)
        {
            case AesEncryptionPlatform.Hardware:
    ...
    /// <param name="aesEncryptionPlatform">The AES encryption platform used to perform the decryption operations.</param>
    ...
    public static async Task DecryptDataAsync(Memory<LimeDataSegment> limeSegments, ulong userId, AesEncryptionPlatform aesEncryptionPlatform, ParallelOptions? po = null)
    {
        ...
        await Parallel.ForAsync(0, limeSegments.Length, po, async (i, _) =>
        {
            ...
            await DeencryptDataAsync(dataAsBytes, cHashPublicKeysResult, aesEncryptionPlatform);
    ...
    /// <param name="aesEncryptionPlatform">The AES encryption platform used to perform the encryption operations on the segment data.</param>
    ...
    public static async Task EncryptDataAsync(Memory<LimeDataSegment> limeSegments, ulong userId, AesEncryptionPlatform aesEncryptionPlatform, ParallelOptions? po = null)
    {
        // Load key into container
        Memory<ulong> cKey1 = new ulong[ContainerCapacityInUlongs];
        await SetupContainerAsync(cKey1, PrivateKey1);
    ...
    /// <param name="aesEncryptionPlatform">The AES encryption platform used to perform decryption operations on the segment data.</param>
    ...
    public static uint? LimepickSegmentBatch(LimeDataSegment limeSegment, AesEncryptionPlatform aesEncryptionPlatform, ParallelOptions? po = null, uint start = uint.MinValue, uint end = uint.MaxValue)
    {
        // TODO: Ensure it doesn't get stuck on false candidates.
        // Load key into container
        ...
        for (var i = start; i <= end; i++)
        {
            ...
            DeencryptData(dataAsBytes, cHashPublicKeysResultAsBytes, aesEncryptionPlatform);
