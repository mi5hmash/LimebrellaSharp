...
{
    /// <summary>
    /// Provides the AES encryption platform supported by the current environment.
    /// </summary>
    ...
    /// <param name="userId">The user identifier used to derive the Steam account ID for decryption. Must be a valid user ID string.</param>
    ...
    public async Task UnpackFilesAsync(string inputDir, string userId, CancellationTokenSource cts)
    {
        ...
        try
        {
            foreach (var file in filesToProcess)
            {
                ...
                try { await limeFile.DecryptSegmentsAsync(steamId, _aesEncryptionPlatform, po); }
                catch (Exception ex)
                {
    ...
    public async Task PackFilesAsync(string inputDir, string userId, CancellationTokenSource cts)
    {
        ...
        try
        {
            foreach (var file in filesToProcess)
            {
                ...
                try { await limeFile.EncryptSegmentsAsync(steamId, _aesEncryptionPlatform, po); }
                catch (Exception ex)
                {
    ...
    /// <summary>
    /// Resigns all encrypted Lime files in the specified directory by decrypting them with the input user ID and re-encrypting them with the output user ID.
    /// </summary>
    /// <param name="inputDir">The path to the directory containing the Lime files to be processed.</param>
    ...
    /// <param name="userIdOutput">The user ID used to re-encrypt the file segments after decryption. Determines the new ownership of the resigned files.</param>
    /// <param name="cts">A CancellationTokenSource used to observe cancellation requests during the resigning process. If cancellation is requested, the operation will terminate early.</param>
    /// <returns>A task that represents the asynchronous resigning operation. The task completes when all eligible files have been processed or the operation is canceled.</returns>
    public async Task ResignFilesAsync(string inputDir, string userIdInput, string userIdOutput, CancellationTokenSource cts)
    {
        ...
        try
        {
            foreach (var file in filesToProcess)
            {
                ...
                {
                    // Try to decrypt file data
                    try { await limeFile.DecryptSegmentsAsync(steamIdInput, _aesEncryptionPlatform, po); }
                    catch (Exception ex)
                    {
                        logger.LogError($"Failed to decrypt the file: {ex.Message}");
                        continue; // Skip to the next file
                ...
                // ENCRYPT
                // Try to encrypt file data
                try { await limeFile.EncryptSegmentsAsync(steamIdOutput, _aesEncryptionPlatform, po); }
                catch (Exception ex)
                {
                    logger.LogError($"Failed to encrypt the file: {ex.Message}");
                    continue; // Skip to the next file
                ...
                logger.LogInfo($"[{progress}/{filesToProcess.Length}] Resigned the [{fileName}] file.");
                progressReporter.Report((int)((double)progress / filesToProcess.Length * 100));
            }
            logger.LogInfo($"[{progress}/{filesToProcess.Length}] All tasks completed.");
        }
