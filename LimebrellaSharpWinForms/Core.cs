    ...
    private Task AsyncLimeFileOperation(OperationType operationType)
    {
        return Task.Run(() =>
        {
            ...
            Parallel.For(0, files.Length, po, ctr =>
            {
                ...
                {
                    _logger.Log(LogSeverity.Error, $"I_{fileId} -> Couldn't load the file. {boolResult.Description}");
                    goto ORDER_66;
                }
                
                // log info about the input file
                _logger.LogDebug(LogSeverity.Information, $"I_{fileId} | {dsssFile.FileName} | {Md5HashFromByteArray(fileDataInput)} | {dsssFile.IsEncrypted}");
                ...
                switch (operationType)
                {
                    ...
                    case OperationType.Unpacking:
                        if (!dsssFile.IsEncrypted)
                        {
                            _logger.Log(LogSeverity.Warning, $"I_{fileId} -> The file is already decrypted.");
                            goto ORDER_66;
                        }
                        dsssFile.Key = SteamIdManager.GetInputNumeric();
                        result = dsssFile.DecryptSegments();
                    ...
                    default:
                        if (dsssFile.IsEncrypted)
                        {
                            _logger.Log(LogSeverity.Warning, $"I_{fileId} -> The file is already encrypted.");
                            goto ORDER_66;
                        }
                        dsssFile.Key = SteamIdManager.GetOutputNumeric();
                ...
                if (!result)
                {
                    _logger.Log(LogSeverity.Warning, $"I_{fileId} -> The provided SteamID does not work for this file.");
                    goto ORDER_66;
                }
                ...
                _logger.LogDebug(LogSeverity.Information, $"O_{fileId} | {dsssFile.FileName} | {Md5HashFromByteArray(fileDataOutput)} | {dsssFile.IsEncrypted}");
